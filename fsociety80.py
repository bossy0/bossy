	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.send(p) # ed invia
				print ("[*] Victims Sent " + str(proxy[0]+":"+proxy[1]) + " ✓", self.counter) # print req + counter
...
class requestproxy(threading.Thread): # la classe del multithreading
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		randomip = str(random.randint(0,0)) + "." + str(random.randint(0,0)) + "." + str(random.randint(0,0)) + "." + str(random.randint(0,0))
